...
## note: Managed fields are v1beta1 after `flux migrate`
```
...
## API server verrr unhappy with us when running `flux migrate` after 2.7.0 upgrade + v1beta1 removed:
```
...
## Managed fields end up empty after post 2.7.0 migrate: (this is what fixes the problem)
```
